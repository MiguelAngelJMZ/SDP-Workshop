...
# MAGIC catalog_name = f"sdp_workshop"
# MAGIC
...
#Ver estado actual final
display(spark.sql(f"SELECT * FROM silver_{clean_username}.customers ORDER BY customer_id;"))

# COMMAND ----------

# Clientes actualizados (con ciudad SF)
display(spark.sql(f"""SELECT customer_id, name, email, city, state
FROM silver_{clean_username}.customers
WHERE city = 'San Francisco'
ORDER BY customer_id;"""))

# COMMAND ----------

# Verificar que eliminados no existan
display(spark.sql(f"""SELECT customer_id 
FROM bronze_{clean_username}.customers_raw
WHERE operation = 'DELETE'
EXCEPT
SELECT customer_id
FROM silver_{clean_username}.customers;"""))
# Debe devolver IDs eliminados (CUST0003, CUST0007)
