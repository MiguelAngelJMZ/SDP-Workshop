...
CREATE OR REFRESH STREAMING TABLE bronze_${user}.customers_raw
...
CREATE OR REFRESH STREAMING TABLE bronze_${user}.customers_clean
  (
...
FROM STREAM bronze_${user}.customers_raw;
...
CREATE OR REFRESH STREAMING TABLE silver_${user}.customers
...
AUTO CDC INTO silver_${user}.customers                 -- Tabla destino a mantener
FROM STREAM bronze_${user}.customers_clean             -- Eventos CDC origen
...
CREATE OR REFRESH MATERIALIZED VIEW gold_${user}.customer_summary
