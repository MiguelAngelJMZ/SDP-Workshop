...
-- MAGIC
-- MAGIC working_dir = f'/Volumes/sdp_workshop/default/raw'
-- MAGIC
...
-- MAGIC username = current_user.split("@")[0]
-- MAGIC clean_username = re.sub(r'[^a-z0-9]', '_', username.lower())
-- MAGIC catalog_name = "sdp_workshop"
-- MAGIC
...
-- MAGIC %python
-- MAGIC display(spark.sql(f"SELECT * FROM bronze_{clean_username}.orders LIMIT 10;"))

-- COMMAND ----------

-- MAGIC %python
-- MAGIC display(spark.sql(f"""SELECT order_id, order_timestamp, customer_id 
-- MAGIC FROM silver_{clean_username}.orders_clean 
...
-- MAGIC %python
-- MAGIC display(spark.sql(f"""SELECT * FROM gold_{clean_username}.order_summary 
