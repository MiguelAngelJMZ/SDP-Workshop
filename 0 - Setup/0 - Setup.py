...
class WorkshopHelper:
    def __init__(self):
        ...
        self.catalog_name = f"sdp_workshop"  # Catálogo con nombre de usuario
        self.default_schema = "default"  # Para almacenamiento de volúmenes
        
        # Definir rutas
        self.volume_schema = f"volume_{clean_username}"
        self.working_dir = f"/Volumes/{self.catalog_name}/{self.default_schema}/raw"
        
        
        # Nombres de esquemas - arquitectura medallion simple
        self.bronze_schema = f"bronze_{clean_username}"
        self.silver_schema = f"silver_{clean_username}"
        self.gold_schema = f"gold_{clean_username}"
