...
CREATE OR REFRESH STREAMING TABLE bronze_${user}.orders -- hereda catálogo por defecto pero especifica esquema bronze
  COMMENT "Datos de pedidos raw ingeridos desde archivos JSON"
  TBLPROPERTIES (
    "quality" = "bronze_miguel_jimenez",
...
CREATE OR REFRESH STREAMING TABLE silver_${user}.orders_clean -- Publica a múltiples catálogos y esquemas
  (
...
SELECT 
  ...
  customer_id,
  notifications
FROM STREAM bronze_${user}.orders;
...
CREATE OR REFRESH MATERIALIZED VIEW gold_${user}.order_summary
...
FROM silver_${user}.orders_clean
